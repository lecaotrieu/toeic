...
<c:forEach var="item" items="${items.listResult}">
    ...
    <p>
        ...
        <c:if test="${items.answerUser !=null}">
            <c:if test="${(item.correctAnswer == 'A') || (item.correctAnswer == 'A' && items.answerUser =='A')}">
                <img src="<c:url value="/template/image/correct.png"/> " alt="">
            </c:if>
            <c:if test="${(items.answerUser =='A') && (items.checkAnswer == true)}">
                <img src="<c:url value="/template/image/incorrect.png"/> " alt="">
            </c:if>
        </c:if>
    </p>
    <p>
    ...
    <input type="hidden" id="exerciseId" name=exerciseId" value="${item.exercise.exerciseId}"/>
</c:forEach>
